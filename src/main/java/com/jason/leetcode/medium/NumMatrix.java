...
public class NumMatrix {
    ...
    public NumMatrix(int[][] matrix) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                prefixSum[i][j] = matrix[i][j] + getValueNullSafe(i - 1, j) + getValueNullSafe(i, j - 1) - getValueNullSafe(i - 1, j - 1);
            }
        }
    }
    ...
    public int sumRegion(int row1, int col1, int row2, int col2) {
        ...
        return prefixSum[row2][col2] - getValueNullSafe(row1 - 1, col2) - getValueNullSafe(row2, col1 - 1) + getValueNullSafe(row1 - 1, col1 - 1);
    }
}
